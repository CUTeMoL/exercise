...
### WAL汇总(复制前提)
summarize_wal = off # [on|off]是否启用WAL汇总进程
wal_summary_keep_time = '10d' # 配置WAL汇总器自动删除旧WAL汇总的时间间隔
## 复制
max_wal_senders = 10 # 最多运训n个并发连接的后备服务器/备份客户端,wal_level必须为replica以上
max_replication_slots = 10 # 指定服务器可以支持的复制槽,wal_level必须为replica以上
```
...
|-|-|
|-f|输出的文件/目录名|
|-F|输出格式[c,d,t,p]对应自定义,目录,tar,富文本|
|-j|多线程导出|
|-Z|压缩|
|-a|只导出数据|
