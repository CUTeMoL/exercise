...
* kuberlet
  ...
  + 健康检查
    - 探针类别
      1) StartupProbe启动探针:诊断和监控容器的启动过程
      2) readinessProbe就绪探针:完成后,才能真正访问pod(StartupProbe完成后开始,直到容器停止,失败时切断容器外部流量)
      3) livenessProbe存活探针:用于检测容器内应用程序的健康状态,如果失败会重启容器(StartupProbe完成后开始,直到容器停止,失败时重启容器)
    - 探针实现方式
      1) ExecAction:容器内执行命令
      2) TCPSocketAction:对容器端口检测
      3) HTTPGetAction:对容器对应端口进行路由检测,一般状态码在`[200,400)`区间
    - 探针的可选项
      1) initialDelaySeconds: 10    # 容器启动后多久开始探测（默认0）
      2) periodSeconds: 10          # 每10秒探测一次（默认10）
      3) timeoutSeconds: 10         # 探测超时时间，超过则视为失败（默认1）
      4) successThreshold: 1        # 失败后连续成功2次才算成功（默认1）
      5) failureThreshold: 3        # 成功后连续失败3次才算失败（默认3）
* cadvisor
  + 收集节点信息传给Metrics server
* kubernetes proxy
...
* kubernetes最小单元
  ...
  + 可以定义一次性容器initcontainers,可以多个,线性顺序启动,如果失败pod会全部重新再从头执行initcontainers,没有readinessProbe就绪探测,靠最后一条命令的返回码判断是否成功
  + 可以通过hook(在容器PostStart初始化完成后、preStop关闭前)执行命令
  + kubelet完成readinessProbe就绪探测后,才能真正访问
  + livenessProbe存活探测如果失败会重启容器
...
* 持久化存储(使用volumeclaimtemplate创建，称为卷申请模板，创建persistentvolume时为每个pod分配并创建一个独一无二的pvc),挂载原来的volume
...
#########故障诊断和调试
  ...
  kubectl exec   #在容器内执行命令
    kubectl exec -it pod_name -c 参数指定容器名称 -- bash -n namespace   #进入容器终端
