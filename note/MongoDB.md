...
```python
db.collection.action1({条件},{列筛选}).action2({条件}) # collection要换成实际的集合名称
```
...
|$type|字段类型|
|$regex|正则表达式|
|$elemMatch|为数组的元素指定多个条件|


```shell
db.collection.find({col_name1:'value1',col_name2:'value2'}) # 查询数据，括号中可填条件，也可以不填
```

```shell
db.collection.find({col_name1:'value1',col_name2:'value2'}{col_name1:1,col_name2:0})  # 查询数据，第一个花括号{条件}，第二个花括号{筛选是否显示，1显示，0不显示}，不输入条件的话第一个花括号可为空{}
```

```shell
db.collection.findOne({col_name1:'value1',col_name2:'value2'}) : 查询数据，只显示第一条数据
```

```python

db.collection.find({col_name1:{'$lt':value1}}) # 查询数据，通过数值的范围查询 
	# $lt|小于	$lte|小于等于	$gt|大于	$gte|大于等于	$ne|不等于

# mongoshell进行时间查询使用ISODate
db.collection.find("time_col": {"$gte": ISODate("2024-01-01T00:00:00Z"),"$lte": ISODate("2025-12-31T00:00:00Z")})

```

```shell
db.collection.find({'col_name1.col_name1.1':'value1.1'}) # 查询2级数据(多维),注意引号
```

```python
db.collection.find({col_name1:{'$all':['value1.1','value1.2']}}) : 查询2级数据(数组)//在[]中的条件都存在的
...
```shell
db.collection.find({'$or':[{条件1},{条件2}]}) # 查询符合条件1或条件2的数据
```

​	例如：`db.information.find({'$or':[{name:'lxw'},{age:27}]})`

```shell
db.collection.find( { dim_cm: { $elemMatch: { $gt: 22, $lt: 30 } } } ) #  dim_cm 数组至少包含一个大于 ($gt) 22 且小于 ($lt) 30 的元素
```

```shell
db.collection.find({条件1，条件2}).count() # 统计次数
```

```shell
db.collection.find({条件1，条件2}).skip(3).limit(2) # 查询符合条件1或条件2的数据，跳过3条，显示2条
```

```shell
db.collection.find({条件1，条件2}).sort({条件3}) # 查询符合条件1或条件2的数据，按条件3进行排序
```
...
db.users.find(
            ...
            "$and": [
               ...
               },
               {
                  "created_at":{
                     ...
                     "$lte": ISODate("2025-12-31T00:00:00Z")
                  }
               }
            ]
         },
   ...
   {"_id":1,"name":1,"balance":1,"is_vip":1,"interests":1,"age":1}
...
```shell
db.collection.updateOne({条件1，条件2},{'$set':条件3}) # 查询符合条件1或条件2的1条数据，修改条件3（仅1条）
```

```shell
db.collection.updateMany({条件1，条件2},{'$set':条件3}) : 查询符合条件1或条件2的所有数据，修改条件3（多条）
```

```shell
db.collection.update({条件1，条件2},{条件3})  # 查询符合条件1或条件2的数据，修改条件3并清空其他字段
```

### 删除数据

```shell
db.collection.deleteOne({条件1，条件2}) # 查询符合条件1或条件2的数据，删除匹配到的一条
```

```shell
db.collection.deleteMany({条件1，条件2}) # 查询符合条件1或条件2的数据，删除匹配到的所有
```

```shell
db.collection.remove({条件1，条件2},justOne:1) # 查询符合条件1或条件2的数据，删除匹配到的一条
```

```shell
db.collection.update({条件1，条件2},{'$unset':{col_name1:0}})  # 查询符合条件1或条件2的数据，删除col_name1这个字段
```
...
# 如果是使用--query或--queryFile查询时间范围使用$date表示时间
# {
#    "created_at":{
