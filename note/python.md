...
| `tarfile_object.add(filename)`                               | 添加文件进`tarfile`,要先移动到目录下,不然路径成为arcname的一部分<br/>`arcname=dir/file`指定压缩包中的名称<br/>`recursive=True`递归<br/>`filter=None`排除 |
...
| 函数                                                                     | 说明                          |
| ---------------------------------------------------------------------- | --------------------------- |
| `ArgumentParser()`                                                     | 创建一个命令行解析器对象`parser_object` |
| `parser_object.add_argument('--option', type=int, default=int_object)` | 添加参数                        |
|                                                                        |                             |
|                                                                        |                             |
|                                                                        |                             |
...
| 函数                   | 功能                 |
| ---------------------- | -------------------- |
| traceback.format_exc() | 接收到的异常堆栈信息 |
...
| 客户端         |                                                              |                                                              |
|                | `docker_socket.containers.list()`                            | 列出所有运行中的容器<br/>`all=True`列出所有创建的容器<br/>`filters=` |
...
| 容器           | `docker_socket.containers.run("image", "CMD", detach=True)`  | 启动一个容器<br/>`detach=True`可以后台运行<br/>`remove|auto_remove=True`运行完成后自动删除·auto_remove·针对后台运行<br/>`entrypoint|CMD=[list]` 执行的命令<br/>`mounts=[list]` 挂载目录到容器,list为挂载对象<br/>`ports={dict}` 端口转发,list为端口转发的对应关系<br/>`restart_policy={dict}`重启策略<br/>`labels={dict}|[list]`打标签<br/>`name`名称 |
...
```python
if __name__ == '__main__':
    uvicorn.run(app="main:app", reload=True, host="127.0.0.1", port=9955)
```

```shell
uvicorn main:app --reload --port 9955 --host 127.0.0.1 # 运行main.py(测试)
```
...
| 函数                                                         | 说明     |
| ------------------------------------------------------------ | -------- |
| `encode(payload: Union[Dict, bytes],key: str,algorithm: str)` | 创建令牌 |
| `decode(jwt: str,key: str,algorithms: List[str])`            | 解析令牌 |
...
定义一个ClientCredential模式认证

```python
class OAuth2ClientCredentialRequestForm:
    """
    自定义一个ClientCredential请求的表单
    """

    def __init__(
        self,
        *,
        grant_type: Annotated[Union[str, None], Form(pattern="clientCredentials")] = None,
