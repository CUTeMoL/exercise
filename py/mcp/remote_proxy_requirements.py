#!/usr/bin/env python3
"""
MCP 远程代理（SDK 版）：本地 stdio ↔ 远程 HTTPS MCP 服务器。

需要第三方依赖: pip install mcp httpx

相比零依赖版（remote_proxy.py）的优势:
- 使用官方 mcp SDK，协议处理更完善（边界情况、参数校验）
- httpx.AsyncClient 持久连接池，复用 TCP/TLS 连接
- 全异步，支持并发场景
...
用法（在 Claude Code 的 mcp.json 中配置）:
{
  "mcpServers": {
    "remote-monitor": {
      "command": "python",
      "args": [
        "C:/path/to/remote_proxy_requirements.py",
        "--host", "172.26.72.248",
        "--port", "8443",
        "--ak", "ak-xxxxxxxx",
        "--sk", "sk-xxxxxxxx",
...
# ── 远程 MCP 客户端（httpx 持久连接池）──────────────────────────────

class RemoteMcpClient:
    """使用 httpx AsyncClient 与远程 MCP 服务器通信，复用 TCP/TLS 连接。"""
...
async def _fetch_tools_with_retry(remote: RemoteMcpClient) -> list[dict]:
    """获取远端工具列表，失败时自动重连重试。"""
    try:
...
async def run():
    parser = argparse.ArgumentParser(description="MCP 远程代理（SDK 版）")
    ...
    try:
        await remote.initialize()
        remote_tools = await _fetch_tools_with_retry(remote)
        ...
        async def handle_list_tools():
            nonlocal remote_tools
            tools = await _fetch_tools_with_retry(remote)
            if tools:
                remote_tools = tools
            return [
                Tool(
